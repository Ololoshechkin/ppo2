...
class ReactiveDatabase(
    ...
    private val conversionRates: CurrencyRates
) {
    ...
    fun getUser(id: Long) = observeGet("users", filter = "userId" to id) {
        User(id, it.getString("mainCurrency").toCurrencyOrThrow())
    }

    fun getProduct(productId: Long) = observeGet("products", filter = "productId" to productId) {
        Product(
            productId,
    ...
    fun getProductsForUser(userId: Long) = getUser(userId).flatMap { user ->
        observeGet("products", filter = null) {
            ...
            Product(
                productId = it.getString("productId").toLong(),
                name = it.getString("name"),
